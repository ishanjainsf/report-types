...
function POSDetailReport() {
    ...
    const reportType = "POS Detail Report"
    ...
    const columns = [
        ...
        {field:"product_qty", headerName:"Quantity"},
        {field:"product_price", headerName:"Product Price (₹)", valueFormatter : (params) => params.value ? params.value.toFixed(2) : params.value},
        {field:"order_date", headerName:"Order Date", filter:"agDateColumnFilter", filterParams:filterParams},
        {field:"order_amount", headerName:"Order Amount (₹)", valueFormatter : (params) => params.value ? params.value.toFixed(2) : params.value },
        ...
        onFilterChanged:(params)=>{
            ...
            setTimeout(()=>{
                params.api.forEachNodeAfterFilter(i=>{
                    ...
                    result.product_price +=i.data.product_price;
                });
    ...
    function updatePinnedRowonDateChange(){
        ...
        rowsData.forEach(data => {
           ...
           result[0].product_price += data.product_price;
        })
    ...
    // calling the api 
    const calendarData = () => {
        handleDataRequest(`reports/getReportData/?hotel_id=12354&report_type=posDetailReport&start_date=${dates[0].startDate}&end_date=${dates[0].endDate}&date_filter_mode=${filterDateType}`)
        .then((res) => setRowsData(res.report_data))
    }   
...
export default POSDetailReport
